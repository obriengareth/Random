...
// mpicc -o eom EOM.c -lm -ffast-math -lfftw3 -w 
...
// Velocity model 
#define velocity_model 0
#define velocityStart 3000
#define velocityEnd    3000

// Data input
#define input_data  "/gpt/MODELLING/EOM/KE/KE_50_mugs_inmosubset_cdp1000_1100.sgy"
#define output_data "/gpt/MODELLING/EOM/KE/csp_test.sgy"

// Survey geometry
#define ox (789868.0)           // origin X
#define oy (265844.0)           // origin Y
#define angleEtoN (0.3)    // orientation of the survey angle from E to N
#define iline_distance  (25.0)    // iline increment in metre
#define xline_distance  (0.0)    // xline increment in metre

// CSP Gather survey
#define oxcsp (802353.875)     // origin csp X in m
#define oycsp (265909.313)     // origin csp Y in m
#define excsp (803604.313)     // end csp X in m
#define eycsp (265915.938)     // end csp Y in m
...
// Data input
#define input_data  "/gpt/MODELLING/EOM/KE/KE_10BB_2012_50_mugs_inmo.sgy"
#define output_data "/gpt/MODELLING/EOM/KE/KE_50_mugs_csp.sgy"
...
// CSP Gather survey
#define oxcsp (789868)        // origin csp X in m
#define oycsp (265844)        // origin csp Y in m
#define excsp (819945)        // end csp X in m
#define eycsp (265999)        // end csp Y in m
#define scaleXY  (0.001)       // scale segy positions
...
//#define oVel_xlines 752  // velocity model xline origin
 ...
 if(velocity_model==0)
 {
    ...
    for(i=0;i<ns; i++)
    {
     vel_t[i] = velocityStart +  (velocityEnd-velocityStart)*(i/ns);
    }
    for(i=0;i<ns; i++)
    vel_t[i]=vel_t[i]*vel_t[i];
 }
    ...
    for(it1=0; it1<ntblock; it1++ ) // loop over indivdual traces in blocks
    {
      if(it1+ntblock*count<nt)
      {
       ...
       for(yc=0; yc<ny; yc++)
       {
         ...
         if( dist<aperture2)
         {
          talpha = round( 2.0*sqrt(h2/vel_t[0])/dt );
          ...
          { 
            ...
            //crossterm2 = 0.0;
            //if( T>=0 && T<ns )
            //crossterm2 = (4.0*x2*h2)/(t*dt*t*dt*vel_t[T]);

            crossterm2 = (4.0*x2*h2)/(t*dt*t*dt*vel_t[t]);
            eq_offset=sqrt( x2 + h2 - crossterm2 );
            ...
            //scaleT = 1.0 - 2.0*sqrt(x2)/eq_offset;
            scaleT = 1.0*scale3 ;//
