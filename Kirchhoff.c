...
// mpicc -o KTM Kirchhoff_time.c -lm -lfftw3 -w -ffast-math
// mpirun -machinefile nodes.pg -n 36 ./KTM
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
// 0 - doesnt do the kirchhoff 1 - full run
#define run_test 1

// Velocity model - (iline xline) units
#define input_vel "/gpt/GEOPRO/BULK/GHANA/Vels_PSDM_RMS_Time_IlineEdited.segy"  // input velocity model
#define NVel_ilines 472  // velocity model length in ilines units
#define NVel_xlines 643  // velocity model number in xlines units
#define dVel_ilines 4    // velocity model iline increment
#define dVel_xlines 8    // velocity model xline increment
#define oVel_ilines 852  // 1704  // velocity model iline origin
#define oVel_xlines 752  // velocity model xline origin

// Data input
#define input_data  "/gpt/MODELLING/KTM/TanoDeep_i1500to1600_c2500to2600.segy"
#define output_data "/gpt/MODELLING/KTM/pstm_gathers.sgy"

// image model output in (iline,xline) coordinates
#define Startiline (1500)         // iline start
#define Endiline   (1600)         // iline end
#define Startxline (2500)         // xline start
#define Endxline   (2600)         // xline end
...
#define oh (200)            // number of offsets

// survey origin and angle
#define ox (444479.29)           // origin X
#define oy (478003.98)           // origin Y
...
#define ntblock (50000)        // read in and process xxx traces at a time
#define number_mpi_jobsX 36   // number of image slices
...
//if(my_rank==0 && iline==2712 && xline==5065)
...
// awful coding reading in to memory twice from same file for different formats
  ...
  for(ih=0; ih<nh; ih++)
  {
   ...
   for(it=0; it<nt+1; it=it+ntblock ) // (it<nt+1) loop over blocks of data
   {
printf("doing traces %d-%d of nt=%d: job %d on node %d\n",it,it+ntblock,nt,job_id,my_rank);
    ...
    for(it1=0; it1<ntblock; it1++ ) // loop over indivdual traces in blocks
    {
      if(it1+ntblock*count<nt)
      {
       ...
       i = htonl(block_header[18 + (60+ns)*it1]) - ox;      j = htonl(block_header[19 + (60+ns)*it1]) - oy;
       sxt =  cosA*i + sinA*j;
       syt = -sinA*i + cosA*j;

       // rotate receiver coordinates into computational grid
       i = htonl(block_header[20 + (60+ns)*it1]) - ox;      j = htonl(block_header[21 + (60+ns)*it1]) - oy;
       gxt =  cosA*i + sinA*j;
       gyt = -sinA*i + cosA*j;
      ...
      // integral 2
       ...
       //dataP[t + trangeMax] = dataP[2*ns-t-2 + trangeMax] ;
        ...
        IX =  (Startiline + ( (nx/2.0 + xslice[job_id]*nx)*INCiline ) )*iline_distance;
        IY =  (Startxline + ( (ny/2.0 + yslice[job_id]*ny)*INCxline ) )*xline_distance;
    ...
    if(dist_IC<aperture+add_on)   //  if loop for not near block
    {
      ...
      for(yc=0; yc<ny; yc++)
      {
         ...
         IX =  (Startiline + ( (xc + xslice[job_id]*nx)*INCiline ) )*iline_distance;
         IY =  (Startxline + ( (yc + yslice[job_id]*ny)*INCxline ) )*xline_distance;
...
/////////////////////////////////////////////////////////////////////////////
  ...
  //////////////////////////////////////////////
   ...
   for(t=1; t<ns; t++)
   {
     ...
     // writing data to image filefmax=
